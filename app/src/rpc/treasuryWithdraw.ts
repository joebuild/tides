...
import { getGlobalTreasuryVaultTA, getVaultAuthority } from "../utils/pda";
...
export const treasuryWithdraw = async (
	...
	const [collateralVault] = getGlobalTreasuryVaultTA(program.programId)
	const [collateralVaultAuthority] = getVaultAuthority(program.programId, collateralVault)

	return program.methods.treasuryWithdraw(amount)
		.accounts(
			{
				admin: provider.wallet.publicKey,
				adminAta: userAta,
				config: configPda,
				treasuryVault: collateralVault,
				treasuryVaultAuthority: collateralVaultAuthority,
